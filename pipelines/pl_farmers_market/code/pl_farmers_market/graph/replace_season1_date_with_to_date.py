...
def replace_season1_date_with_to_date(spark: SparkSession, in0: DataFrame) -> DataFrame:
    from pyspark.sql.functions import col, substring, to_date
    out0 = in0.withColumn(
        "ToDate",
        when(col("ToDate").isNull(), to_date(substring(col("Season1Date"), 1, 10), "MM/dd/yyyy"))\
